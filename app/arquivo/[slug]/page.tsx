...
export async function generateStaticParams() { return cartas.map((c) => ({ slug: c.slug })); }

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const carta = getCartaBySlug(slug);
  if (!carta) return { title: "Carta não encontrada" };
  return { title: carta.titulo, description: carta.excerpt, openGraph: { title: `${carta.titulo} — Almanaque Contemporâneo`, description: carta.excerpt, type: "article", publishedTime: carta.data } };
}
